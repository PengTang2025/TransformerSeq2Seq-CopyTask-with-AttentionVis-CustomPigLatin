...
def merge_images(image_name, image_dir, output_path):
    ...
    for i in range(8):
        # 读取每个头的注意力热图
        # 格式：word_encoder_attn_head_i.png, word_decoder_self-attn_head_i.png, word_decoder_cross-attn_head_i.png
...
if __name__ == "__main__":
    image_dir = "test_visualization"  # 图像所在目录  
    output_path = "merged_images"  # 输出目录
    os.makedirs(output_path, exist_ok=True)  # 确保输出目录存在
    # 文件名的格式是word_encoder_attn_head_i.png, word_decoder_self-attn_head_i.png, word_decoder_cross-attn_head_i.png
    # 先读文件名，获取word列表
    words = set()
    for filename in os.listdir(image_dir):
        word = filename.split('_')[0]  # 分割文件名
